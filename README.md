...
dotnet test  MedSupplyOps.slnx --nologo                                    # 67 條測試
...
所以 [`scripts/mutation-probe.ps1`](scripts/mutation-probe.ps1) 會自動把實作**故意改壞 7 次**，
...
```
產品碼 1,572 行 ｜ 測試碼 1,506 行 ｜ SQL 543 行
67 條測試（Domain 48 + Integration 19，整合測試全部跑真實 Oracle）
7 支程式碼探針 + 5 支資料庫探針 + 1 道 ER 圖漂移關卡
```
