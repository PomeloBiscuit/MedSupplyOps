...
public sealed class Code128BarcodeEncoderTests
{
    ...
    /// ★ 整張對照表的快照。
    ///
    /// 黃金值只涵蓋少數符號，結構不變量又抓不到「兩個合法圖案互換」——覆核時互換 70 與 71，
    /// 這個檔案裡其他測試全部照樣綠。所以把 107 格整張凍結：任何一格被改，不論黃金值有沒有涵蓋，都會紅。
    ///
    /// 這份快照不是「照著實作抄一份來比自己」：它是在
    /// <c>Code128ZxingCrossValidationTests.Every_symbol_in_the_table_is_verified_by_decoding_through_ZXing</c>
    /// 全綠——107 個符號每一個都被 ZXing 實際解碼過——之後才凍結的。
    /// 移除 ZXing 之後，這條就是對照表唯一的守衛，**不要跟著實作一起改它**；
    /// 真的要改編碼表，必須先重新取得獨立驗證。
    /// </summary>
    [Fact]
    public void The_full_symbol_table_matches_the_snapshot_verified_through_ZXing()
    {
        ...
        Assert.Equal(107, verifiedSnapshot.Length);
        Assert.Equal(verifiedSnapshot, Code128BarcodeEncoder.SymbolPatterns);
    }
