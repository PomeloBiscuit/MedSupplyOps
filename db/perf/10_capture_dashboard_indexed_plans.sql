-- 儀表板讀取路徑調校後：V007 已套用，在完全相同資料、bind、SELECT 與回傳列數下連跑兩次。
...
SELECT /*+ GATHER_PLAN_STATISTICS */ /* WO_S_EXPIRED_INDEXED_RUN_1 */ COUNT(*)
  FROM stock_lots l
 INNER JOIN items i ON i.item_id = l.item_id
 WHERE i.is_deleted = 0 AND l.quantity > 0 AND l.expiry_date < TO_DATE(:asOf, 'YYYY-MM-DD');

SELECT /*+ GATHER_PLAN_STATISTICS */ /* WO_S_EXPIRED_INDEXED_RUN_2 */ COUNT(*)
  FROM stock_lots l
 INNER JOIN items i ON i.item_id = l.item_id
 WHERE i.is_deleted = 0 AND l.quantity > 0 AND l.expiry_date < TO_DATE(:asOf, 'YYYY-MM-DD');
SET TERMOUT ON
