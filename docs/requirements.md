...
| **FR-103** | 已實作 | `src/MedSupplyOps.Web/Controllers/UsersController.cs`、`src/MedSupplyOps.Web/Views/Users`、`AuthorizationPolicies.UserManage`（清單與角色／狀態篩選、新增、編輯、停用／啟用、一次性顯示重設密碼；Identity、最後管理員守衛、SecurityStamp、User audit） | `T1_last_administrator_and_self_guards_block_all_four_direct_posts`、`T2_requester_and_storekeeper_are_denied_by_every_user_management_endpoint`、`T3_disabled_user_gets_generic_login_failure_then_can_login_after_enable`、`T4_reset_password_is_one_time_invalidates_old_password_and_session_and_audits_no_secret`、`T5_requester_without_department_is_rejected_on_create_and_edit`、`Create_edit_and_filters_persist_identity_values_and_audit_role_and_department_without_password` |
